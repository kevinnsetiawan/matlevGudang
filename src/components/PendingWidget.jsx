  ...
  return (
    <div className="dashboard-pending-widget" style={{...sty.card,borderLeft:`4px solid #f59e0b`,marginBottom:16}}>
      ...
      {myPendingApprovals.slice(0,4).map(t=>{
        ...
        const label = t.docType==="TUG5"?(t.sourceType==="ULTG"?"Reservasi Material":t.keteranganUmum||"Permintaan Material"):t.docType==="TUG7"?`TUG-7 → ${t.unitPenerima||"UPT"}`:t.namaPekerjaan||"-";
        return (
          <div key={t.id} className="dashboard-pending-widget__row" style={{borderBottom:`1px solid ${C.border}`}}>
            <div className="dashboard-pending-widget__copy">
              <div style={{fontSize:12,fontWeight:600}}>{label}</div>
              <div style={{fontSize:12,color:"#0098da"}}>{t.docType.replace("TUG","TUG-")} • {docNo}</div>
            </div>
            <button className="dashboard-widget-action" style={sty.btn("primary","sm")} onClick={()=>setTab("approval")}>Review</button>
          </div>
        );
      })}
