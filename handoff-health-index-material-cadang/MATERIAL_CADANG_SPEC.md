...
## 19. Addendum 2026-07-03: Health Index Material Cadang + AI Insight

Fitur Health Index ditambahkan di atas hasil ABC/policy existing. Angka resmi tetap dihitung lokal di `App.jsx` agar audit-able dan bisa dijalankan tanpa API key AI. AI dipakai setelah hasil deterministic selesai untuk memberi insight manajemen, diagnosis risiko, validasi kualitas data, dan rekomendasi tindakan.

Storage fallback/CLOUD baru:

- `pln_material_cadang_health_v1`
- `pln_material_cadang_ai_insights_v1`

Storage legacy tetap dipertahankan:

- `pln_material_cadang_v1`

Domain data Supabase baru ada di `supabase/schema.sql` section `20. MATERIAL_CADANG_HEALTH_INDEX`:

- `material_cadang_imports`
- `material_cadang_analysis_runs`
- `material_cadang_health_results`
- `material_cadang_ai_insights`
- `material_cadang_apply_audit`

Helper utama di `App.jsx`:

- `calculateMaterialCadangHealthIndex(result, context)`
- `enrichMaterialCadangHealthResults(results)`
- `buildMaterialCadangAiContext(run, results, stocks, katalogList, txns)`
- `generateMaterialCadangAiInsights(run, results, stocks, katalogList, txns)`
...
Komponen risiko deterministic:

- stok saat ini dibanding recommended qty
- kelas ABC dan policy inventory
- lead time
- failure, penggantian, emergency 5 tahun
- gap nilai rupiah
- data confidence
- warning import, nama beda, cluster kosong, harga kosong, lead time/TTF tidak valid

Kontrak AI:

- provider memakai Groq API existing via `VITE_GROQ_API_KEY`
- output wajib JSON
- AI tidak boleh mengubah `healthIndex`, `recommendedQty`, `minQty`, stok, approval, atau data import
- jika API key tidak tersedia, Health Index tetap berjalan dan UI menampilkan fallback `AI insight belum tersedia`

JSON AI yang diharapkan:

```json
{
  "executiveSummary": "Ringkasan manajemen singkat",
  "topRisks": [],
  "dataQualityFindings": [],
  "recommendedActions": [],
  "procurementPriority": [],
  ...
  "materialInsights": [
    {
      "noKatalog": "0000000000",
      "nama": "Nama material",
      "diagnosis": "Diagnosis singkat",
      "recommendation": "Ajukan Apply Min Qty",
      "confidence": 0.85
    }
  ]
}
...
Menu Material Cadang sekarang memiliki tampilan management-friendly:

- `Dashboard`: KPI total dianalisis, gap qty, nilai gap, status Health Index, rata-rata confidence, dan top 10 prioritas.
- `Health Index`: tabel material dengan Health Index, status, confidence, gap, nilai gap, dan rekomendasi.
- `AI Insight`: executive summary, top risk, temuan kualitas data, rekomendasi, dan daftar validasi.
- `Import & Hitung`: upload file, hitung deterministic, lalu panggil AI insight bila tersedia.
- `Apply Min Qty`: pengajuan Admin/TL dan approval Asman.

Audit apply:

- Admin/TL dapat mengajukan `Apply Min Qty`.
- Asman menyetujui atau menolak.
- Approval yang disetujui memperbarui `katalogList.minQty`.
- Semua request/approve/reject dicatat juga di `materialCadangHealthData.applyAudit` dan disiapkan untuk tabel `material_cadang_apply_audit`.

Paket file handoff ke Claude/Supabase:
