...
**Bot Telegram fase 2 — SEBAGIAN, BELUM DI-COMMIT/DEPLOY.** DB: `tg_allowed_users.upt_id` ditambah+backfill (id1=NULL global, id6/7/8=UPT-SBY; migration `20260808_tg_allowed_users_per_upt.sql` applied). Kode (uncommitted): `telegram-webhook/index.ts` scope RAG via `p_upts` (NULL=global lihat semua, else `[upt]` +chunk nasional; RPC tak berubah); form `TelegramWhitelistPanel.jsx` + `AIAgentPage.jsx` tambah dropdown scope wajib (Global/per-UPT) di pendaftaran user baru. **SISA:** (a) verifikasi browser dropdown scope; (b) **deploy manual** edit `telegram-webhook` ke self-host (gated); (c) commit. **Gap diketahui (ditandai `ponytail:` di index.ts):** `buildWarnotoStateContext` masih kirim ringkasan stok NASIONAL ke user scoped (blob `buildWarnotoStateSnapshot` App.jsx ~L4510 tanpa `upt_id` per item) — user pilih TERIMA dulu/restrukturisasi? (belum dijawab).
