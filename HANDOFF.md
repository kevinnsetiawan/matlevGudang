...
- 2026-08-07 Claude: Refactor App.jsx tranche-2 — domain OCR/denah+koordinat blok → `src/hooks/useDenahOcr.js` (relokasi murni, build LULUS), belum verifikasi browser/commit. Tranche-1 (Maturity) sudah LULUS browser stlh 1 bugfix TDZ.
- 2026-08-07 Claude: Multi-UPT 3-tier SELESAI & diverifikasi user di browser — scoping UI + label tier (PLN Pusat/UIT JBM/nama UPT) + dokumen per-UPT (`c0d2817`,`5c05dc9`), fix 403 seed gudang/lokasi (`aad4704`), RAG Pak War per-UPT applied + tagging/reader + sync scope-safe + tombol Sync KB akun nasional/UIT (`3642fa0`,`a0b7989`,`47eb5f7`). Filter UPT di Data Stok (`c49c937`). Spec Kit dijadikan tool global (`d6a8b34`).
