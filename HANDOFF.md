...
- **Kapasitas → Peta Utilisasi — BELUM COMMIT (3 file: `App.jsx`, `KapasitasGudangTab.jsx`, `PetaGudangTab.jsx`), tunggu verifikasi browser.** Tambah filter UPT peta (`petaUptFilter` by `gudang.uptId`, opsi dari `getScopeUptIds`, oper `uptList` ke KapasitasGudangTab). Fix stale: PetaGudangTab useEffect kosongkan `selectedGudangId` saat `gudangList` kosong (UPT tanpa gudang tak nyangkut sub-gudang UPT lain). Redesign toolbar: satukan selector jadi 1 panel berlabel drill-down (1.UPT → 2.Gudang → 3.Denah) + checkbox, hapus `capacity-filterbar` orphan. Build LULUS. **SISA: user verifikasi browser lalu commit+push.**

- **BUG BELUM DIPERBAIKI — Cari Foto (Data Stok) tak terfilter UPT.** Lihat "Langkah berikutnya".
...
**Lanjutan multi-UPT UI (Vendor C lanjut di sini):**
1. **Verifikasi browser + commit Peta Utilisasi** (3 file uncommitted di atas): login UIT/Pusat → Kapasitas → Peta Utilisasi, cek dropdown UPT muncul & selector Gudang/sub-gudang menyempit sesuai UPT; login 1-UPT dropdown tak muncul. OK → `git commit` + `git push`.
2. **FIX Cari Foto Data Stok tak terfilter UPT (tukang-senior — logic baru).** Root cause di `App.jsx` `runPhotoSearch()` (~L2175): DUA mode abaikan scope+`stockUptFilter`. (a) nameplate: `matchNameplateAll(text, katalogList, stocks)` pakai `stocks` MENTAH (semua UPT); (b) bentuk: RPC `match_stock_photos` dipanggil `p_upt: null`. Hasil kedua mode berbentuk `{katalog, similarity}` (dikunci per `katalog`, dirender `DataStokTab.jsx:96-97` via `enrichedStocks.find(katalog)`). Karena `katalog` = master LINTAS-UPT, filter yang benar = "katalog yang punya stok dalam scope efektif". **Fix:** di `runPhotoSearch`, hitung `allowedKatalog = new Set(stocks.filter(s => stockUptFilter ? uptOf(s)===stockUptFilter : inScopeUpt(uptOf(s), getScopeUptIds(currentUser, uptList))).map(s=>String(s.katalog)))` dengan `uptOf(s)` = map stok→gudang→uptId (pola `App.jsx:5062-5065`: `lokasiList.find(l=>l.id===s.lokasiId)?.gudangId || s.gudangId` → `gudangList.find(g=>g.id===gid)?.uptId`). Lalu filter hasil KEDUA mode ke `allowedKatalog` sebelum `setPhotoSearchResults`. `stockUptFilter`/`currentUser`/`uptList`/`gudangList`/`lokasiList` semua ada di closure handler. Reuse `getScopeUptIds`/`inScopeUpt` dari `src/lib/roles.js` (sudah di-import App.jsx). Client-side, TANPA ubah RPC/skema. Verifikasi: login UIT/Pusat, pilih tab UPT tertentu di Data Stok, Cari Foto (bentuk & nameplate) → hasil hanya katalog yang punya stok di UPT itu.
...
2. Apply 4c + verifikasi: `ssh minipc-gudang-home 'docker exec -i supabase-db psql -U postgres --single-transaction' < supabase/migrations/20260806_tutup_tulis_anon_gelombang4c.sql`, lalu `... psql -U postgres -At < supabase/verify_gelombang4c.sql`. Kalau GRANT masih memunculkan baris, tersangka `alter default privileges`.
3. Baru apply 4a: `ssh minipc-gudang-home 'docker exec -i supabase-db psql -U postgres -v ON_ERROR_STOP=1' < supabase/migrations/20260805_multi_upt_rls_gelombang4a.sql`, lalu `verify_gelombang4a.sql`.
4. Cek policy lama benar-benar hilang (`select policyname from pg_policies where tablename in (...)`).
5. Lanjut Gelombang 4b (skema, butuh proposal user).
